...
```shell
msbot clone services --name "EnterpriseBotLab" --luisAuthoringKey "YOUR_AUTHORING_KEY" --folder "DeploymentScripts\YOUR_LOCALE_FOLDER" --location "westus"
```
...
```shell
az bot publish --name "EnterpriseBotLab" --resource-group "EnterpriseBotLab"
```
